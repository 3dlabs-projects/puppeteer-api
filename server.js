...
async function connectBrowser() {
  return puppeteer.connect({
    browserWSEndpoint: 'ws://172.17.0.1:3000',
...
app.post('/scrape', async (req, res) => {
  ...
  if (!startUrl || !category) {
    return res.status(400).json({ success: false, error: 'startUrl or category missing' });
  }
...
app.listen(4000, '0.0.0.0', () => {
  console.log('ðŸš€ Universal Puppeteer Service running on port 4000');
});
